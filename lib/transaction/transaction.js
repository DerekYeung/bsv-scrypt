...
Transaction.prototype.getSerializationError = function (opts) {
  ...
  return unspentError || this._hasDustOutputs(opts)
}
...
Transaction.prototype.getSignatures = function (privKey, sigtype) {
  ...
  const inputs = (this.inputs || []).filter(node => {
    return !node.script || node.script.chunks.length === 0;
  })
  _.each(this.inputs, function forEachInput (input, index) {
    const needSign = !input.script || input.script.chunks.length === 0;
    if (!needSign) {
      results.push(null);
    } else {
